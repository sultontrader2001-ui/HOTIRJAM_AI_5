"""Trade Decision Policy — BUY Strategy Phase 4 (Liquidity Filter).

BUY is internally eligible when assessment is READY, structured MarketContext
fields match, Physics velocity/acceleration are positive, and Liquidity
shift/imbalance both indicate BUY. Summary text is never inspected.
Sprint 23 still emits NO_TRADE only. SELL remains unavailable.
"""
...
def matches_buy_strategy(
    ...
    """Return True when context + physics + liquidity match BUY Phase 4.

    Uses structured fields only. Never inspects summary text.
    """
...
def is_buy_eligible(
    ...
    """BUY eligible when assessment is READY and Phase-4 strategy matches.

    Eligibility does not emit BUY in Sprint 23.
    """
    return (
        ...
        and matches_buy_strategy(context, physics, liquidity)
    )
...
def apply_trade_decision_policy(
    ...
    *,
    ...
    """Apply BUY Phase-4 strategy with liquidity filter; emit NO_TRADE only.
    ...
    elif not matches_buy_strategy(context, physics, liquidity):
        reason = STRATEGY_NOT_SATISFIED_REASON
    else:
        assert is_buy_eligible(assessment, context, physics, liquidity)
